...
/// struct for typed errors of method `create_or_update_sap_entitlement`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateOrUpdateSapEntitlementError {
    Status403(),
...
pub async fn create_or_update_sap_entitlement(configuration: &configuration::Configuration, api_version: &str, sap: Option<crate::models::Sap>) -> Result<crate::models::Sap, Error<CreateOrUpdateSapEntitlementError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/sap/entitlement", configuration.base_path);
    ...
    local_var_req_builder = local_var_req_builder.json(&sap);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateOrUpdateSapEntitlementError> = serde_json::from_str(&local_var_content).ok();
