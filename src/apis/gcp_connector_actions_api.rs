...
/// struct for typed errors of method `create_gcp_connector_action`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateGcpConnectorActionError {
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method `describe_gcp_connector_action`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DescribeGcpConnectorActionError {
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
}
...
pub async fn create_gcp_connector_action(configuration: &configuration::Configuration, gcp_connector_id: i32, api_version: &str, action: crate::models::Action) -> Result<crate::models::Action, Error<CreateGcpConnectorActionError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/gcpconnectors/{gcpConnectorID}/actions", configuration.base_path, gcpConnectorID=gcp_connector_id);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateGcpConnectorActionError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn describe_gcp_connector_action(configuration: &configuration::Configuration, gcp_connector_id: i32, action_id: i64, api_version: &str) -> Result<crate::models::Action, Error<DescribeGcpConnectorActionError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/gcpconnectors/{gcpConnectorID}/actions/{actionID}", configuration.base_path, gcpConnectorID=gcp_connector_id, actionID=action_id);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<DescribeGcpConnectorActionError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
