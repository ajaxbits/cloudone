...
/// struct for typed errors of method `describe_current_account_details`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DescribeCurrentAccountDetailsError {
    Status403(),
    Status404(),
...
pub async fn describe_current_account_details(configuration: &configuration::Configuration, api_version: &str) -> Result<crate::models::AccountDetails, Error<DescribeCurrentAccountDetailsError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/accountdetails/current", configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<DescribeCurrentAccountDetailsError> = serde_json::from_str(&local_var_content).ok();
