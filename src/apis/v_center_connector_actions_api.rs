...
/// struct for typed errors of method `create_v_center_connector_action`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateVCenterConnectorActionError {
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method `describe_v_center_connector_action`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DescribeVCenterConnectorActionError {
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
}
...
pub async fn create_v_center_connector_action(configuration: &configuration::Configuration, virtual_id: i32, api_version: &str, v_center_connector_action: crate::models::VCenterConnectorAction) -> Result<crate::models::VCenterConnectorAction, Error<CreateVCenterConnectorActionError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/vcenterconnectors/{virtualID}/actions", configuration.base_path, virtualID=virtual_id);
    ...
    local_var_req_builder = local_var_req_builder.json(&v_center_connector_action);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateVCenterConnectorActionError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn describe_v_center_connector_action(configuration: &configuration::Configuration, virtual_id: i32, action_id: i64, api_version: &str) -> Result<crate::models::VCenterConnectorAction, Error<DescribeVCenterConnectorActionError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/vcenterconnectors/{virtualID}/actions/{actionID}", configuration.base_path, virtualID=virtual_id, actionID=action_id);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<DescribeVCenterConnectorActionError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
